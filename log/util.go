...
func SubmitFrontendConsoleMessages(ctx context.Context, projectID int, sessionSecureID string, messages string) error {
	...
	for _, row := range logRows {
		span, _ := scout.StartTraceWithoutResourceAttributes(
			ctx, scout.ScopedKey("ctx", ptr.String("-")), []trace.SpanStartOption{trace.WithSpanKind(trace.SpanKindClient)},
			attribute.String(scout.SourceAttribute, "frontend"),
			attribute.String(scout.ProjectIDAttribute, strconv.Itoa(projectID)),
			attribute.String(scout.SessionIDAttribute, sessionSecureID),
		)
		...
		span.AddEvent(scout.LogEvent, trace.WithAttributes(attrs...), trace.WithTimestamp(time.UnixMilli(row.Time)))
		if row.Type == "error" {
...
func submitVercelLog(ctx context.Context, projectID int, log VercelLog) {
	...
	span.AddEvent(scout.LogEvent, trace.WithAttributes(attrs...), trace.WithTimestamp(time.UnixMilli(log.Timestamp)))
	if log.Type == "error" {
...
func SubmitHTTPLog(ctx context.Context, projectID int, lg Log) error {
	span, _ := scout.StartTraceWithoutResourceAttributes(
		ctx, "scout-ctx", []trace.SpanStartOption{trace.WithSpanKind(trace.SpanKindClient)},
		attribute.String(scout.ProjectIDAttribute, strconv.Itoa(projectID)),
	)
	defer scout.EndTrace(span)
	...
	span.AddEvent(scout.LogEvent, trace.WithAttributes(attrs...), trace.WithTimestamp(t))
	if lg.Level == "error" {
